...
如果电脑没有 Python 3.11，一键工具会从 `python.org` 下载官方安装程序，验证固定 SHA-256 后按当前用户范围安装，不修改系统级 PATH；检测到的真实 `python.exe` 路径会写入本机配置。转写模型首次运行还可能下载 Whisper 模型文件，这是正常现象，耗时取决于网络速度。
